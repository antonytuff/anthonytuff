...
def extract_toc(html_body):
    ...
    modified_html = re.sub(r'<h([1-4])(\s[^>]*)?>(.+?)</h[1-4]>', collect_heading, html_body)
...
def build_listing(posts, section, title, description):
    ...
    (OUTPUT / f"{section}.html").write_text(page, encoding='utf-8')
...
def build_resume():
    ...
    (OUTPUT / "resume.html").write_text(page, encoding='utf-8')
...
def build_contact():
    ...
    (OUTPUT / "contact.html").write_text(page, encoding='utf-8')
...
def build():
    ...
    (OUTPUT / "search-index.json").write_text(json.dumps(search_data, indent=2), encoding='utf-8')
